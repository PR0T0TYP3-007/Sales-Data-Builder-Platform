...
const createCompany = async (name, phone, address, industry = null, description = null) => {
  const query = `
    INSERT INTO companies (name, phone, address, industry, description, product_id)
    VALUES ($1, $2, $3, $4, $5, $6)
    RETURNING *;
  `;
  const result = await pool.query(query, [name, phone, address, industry, description, 1]);
